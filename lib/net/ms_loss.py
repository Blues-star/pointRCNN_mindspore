...
class net_with_loss(nn.Cell):
    ...
    def construct(self, *cols):
        ...
        if cfg.RPN.ENABLED:
            ...
            if not cfg.RPN.FIXED:
                ...
                # rpn_reg_label = ms.Tensor.from_numpy(rpn_reg_label).astype(ms.float32)
            ...
            for key, val in data.items():
                if key != 'sample_id':
                    input_data[key] = ms.Tensor.from_numpy(val).contiguous().astype(ms.float32)
    ...
    def get_rpn_loss(self, rpn_cls, rpn_reg, rpn_cls_label, rpn_reg_label, tb_dict):
        
        if cfg.RPN.LOSS_CLS == 'DiceLoss':
            rpn_cls_loss_func = loss_utils.DiceLoss(ignore_target=-1)
        elif cfg.RPN.LOSS_CLS == 'SigmoidFocalLoss':
            rpn_cls_loss_func = loss_utils.SigmoidFocalClassificationLoss(alpha=cfg.RPN.FOCAL_ALPHA[0],
        ...
        if cfg.RPN.LOSS_CLS == 'DiceLoss':
            ...
            # _min_va = ms.Tensor(1.0,ms.float32)
            cls_weights = cls_weights / ops.clip_by_value(pos_normalizer, _min_va)
            ...
            batch_loss_cls = bce(ops.sigmoid(rpn_cls_flat), rpn_cls_label_target,
                                                    weight)
            cls_valid_mask = (rpn_cls_label_flat >= 0).astype(ms.float32)
            _min_va = ms.Tensor(1.0,ms.float32)
            rpn_loss_cls = (batch_loss_cls * cls_valid_mask).sum() / ops.clip_by_value(cls_valid_mask.sum(), _min_va)
        else:
            raise NotImplementedError
        ...
        if fg_sum != 0:
            reg_shape = rpn_reg.shape
            rpn_reg_label_shape = rpn_reg_label.shape
            # ms.Tensor.expand_dims(-1)
        ...
        tb_dict.update({'rpn_loss_cls': rpn_loss_cls.asnumpy().item(), 'rpn_loss_reg': rpn_loss_reg.asnumpy().item(),
                        'rpn_loss': rpn_loss.asnumpy().item(), 'rpn_fg_sum': fg_sum, 'rpn_loss_loc': loss_loc.asnumpy().item(),
                        'rpn_loss_angle': loss_angle.asnumpy().item(), 'rpn_loss_size': loss_size.asnumpy().item()})

        return rpn_loss

    def get_rcnn_loss(self, ret_dict, tb_dict):
        if cfg.RCNN.LOSS_CLS == 'SigmoidFocalLoss':
            cls_loss_func = loss_utils.SigmoidFocalClassificationLoss(alpha=cfg.RCNN.FOCAL_ALPHA[0],
            ...
            cls_loss_func = nn.CrossEntropyLoss(weight=cls_weight,ignore_index=-1, reduce=None)
        else:
            raise NotImplementedError
        ...
        if cfg.RCNN.LOSS_CLS == 'SigmoidFocalLoss':
            ...
            rcnn_cls_flat = rcnn_cls.view(-1)
            batch_loss_cls = ops.BinaryCrossEntropy('none')(ops.sigmoid(rcnn_cls_flat), cls_label, ms.numpy.ones_like(cls_label,cls_label.dtype))
            cls_valid_mask = (cls_label_flat >= 0).astype(ms.float32)
            
            rcnn_loss_cls = (batch_loss_cls * cls_valid_mask).sum() / ops.clip_by_value(cls_valid_mask.sum(), _min_va)
            ...
            #  ms.Tensor.mean()
            rcnn_loss_cls = (batch_loss_cls.mean(axis=1) * cls_valid_mask).sum() / normalizer

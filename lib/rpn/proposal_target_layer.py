...
class ProposalTargetLayer(nn.Cell):
    ...
    def construct(self, **input_dict):
        ...
        for k in range(batch_size):
            ...
            batch_gt_of_rois[k] = kitti_utils.rotate_pc_along_y_torch(batch_gt_of_rois[k].expand_dims(1),
                                                                      roi_ry[k]).squeeze(1)
    ...
    def sample_rois_for_rcnn(self, roi_boxes3d, gt_boxes3d):
        ...
        fg_rois_per_image = int(ms.numpy.round(cfg.RCNN.FG_RATIO * cfg.RCNN.ROI_PER_IMAGE))
        ...
        batch_gt_of_rois = ms.numpy.zeros((batch_size, cfg.RCNN.ROI_PER_IMAGE, 7))
        ...
        for idx in range(batch_size):
            ...
            if fg_num_rois > 0 and bg_num_rois > 0:
                ...
                # sampling fg
                rand_num = ms.numpy.floor(np.random.rand(cfg.RCNN.ROI_PER_IMAGE) * fg_num_rois)
                # rand_num = ms.Tensor.from_numpy(rand_num).type_as(gt_boxes3d).long()
                rand_num = ms.Tensor.from_numpy(rand_num).astype(gt_boxes3d.dtype)
    ...
    def aug_roi_by_noise_torch(self, roi_boxes3d, gt_boxes3d, iou3d_src, aug_times=10):
        ...
        iou_of_rois = ms.numpy.zeros((roi_boxes3d.shape[0]), ms.float32).astype(gt_boxes3d.dtype)
    ...
    def random_aug_box3d(box3d):
        ...
        if cfg.RCNN.REG_AUG_METHOD == 'single':
            ...
            angle_rot = (rand(1) - 0.5) / (0.5 / (np.pi / 12))  # [-pi/12 ~ pi/12]
            ...
            # aug_box3d = ms.Tensor.from_numpy(aug_box3d).type_as(box3d)
    ...
    def data_augmentation(self, pts, rois, gt_of_rois):
        ...
        angles = (rand((batch_size, boxes_num)) - 0.5 / 0.5) * (np.pi / cfg.AUG_ROT_RANGE)
        ...
        temp_beta = ops.atan2(temp_z, temp_x)
        roi_alpha = -ops.Sign()(temp_beta) * np.pi / 2 + temp_beta + temp_ry  # (B, M)

        for k in range(batch_size):
            pts[k] = kitti_utils.rotate_pc_along_y_torch(pts[k], angles[k])
            # Tensor.expand_dims()
            gt_of_rois[k] = kitti_utils.rotate_pc_along_y_torch(gt_of_rois[k].expand_dims(axis=1), angles[k]).squeeze(axis=1)
            rois[k] = kitti_utils.rotate_pc_along_y_torch(rois[k].expand_dims(axis=1), angles[k]).squeeze(axis=1)

            # calculate the ry after rotation
            temp_x, temp_z = gt_of_rois[:, :, 0], gt_of_rois[:, :, 2]
            temp_beta = ops.atan2(temp_z, temp_x)
            gt_of_rois[:, :, 6] = ops.Sign()(temp_beta) * np.pi / 2 + gt_alpha - temp_beta

            temp_x, temp_z = rois[:, :, 0], rois[:, :, 2]
            temp_beta = ops.atan2(temp_z, temp_x)
            rois[:, :, 6] = ops.Sign()(temp_beta) * np.pi / 2 + roi_alpha - temp_beta
        ...
        gt_of_rois[:, :, 0:6] = gt_of_rois[:, :, 0:6] * scales.expand_dims(axis=2)
        ...
        src_ry = gt_of_rois[:, :, 6]
        ry = (flip_flag == 1).astype(ms.float32) * src_ry + (flip_flag == -1).astype(ms.float32) * (ops.Sign()(src_ry) * ms.numpy.pi - src_ry)
        gt_of_rois[:, :, 6] = ry

...
class KittiRCNNDataset(KittiDataset):
    ...
    def get_rpn_features(rpn_feature_dir, idx):
        ...
        if cfg.RCNN.USE_SEG_SCORE:
            ...
            # rpn_seg_score = torch.sigmoid(Tensor.from_numpy(rpn_seg_score)).asnumpy()
            rpn_seg_score = ms.ops.Sigmoid()(ms.Tensor.from_numpy(rpn_seg_score)).asnumpy()
        else:
    ...
    def get_rpn_sample(self, index):
        ...
        if cfg.GT_AUG_ENABLED and self.mode == 'TRAIN': #True
    ...
    def get_rcnn_sample_info(self, roi_info):
        ...
        boxes_pts_mask_list = roipool3d_utils.pts_in_boxes3d_cpu(Tensor.from_numpy(rpn_xyz),
                                                                 Tensor.from_numpy(pooled_boxes3d))
        pt_mask_flag = (boxes_pts_mask_list[0].asnumpy() == 1)
        ...
        if aug_pts.shape[0] == 0:
            ...
            if cfg.RCNN.USE_MASK:
                if cfg.RCNN.MASK_TYPE == 'seg':
                    ...
                    pts_mask = roipool3d_utils.pts_in_boxes3d_cpu(Tensor.from_numpy(aug_pts),
                                                                  Tensor.from_numpy(aug_roi_box3d.reshape(1, 7)))
                    pts_mask = (pts_mask[0].asnumpy() == 1).astype(np.float32)
                else:
                    raise NotImplementedError
    ...
    def canonical_transform_batch(pts_input, roi_boxes3d, gt_boxes3d):
        ...
        # rotate to the direction of head
        gt_boxes3d_ct = kitti_utils.rotate_pc_along_y_torch(Tensor.from_numpy(gt_boxes3d_ct.reshape(-1, 1, 7)),
                                                            Tensor.from_numpy(roi_ry)).asnumpy().reshape(-1, 7)
        gt_boxes3d_ct[:, 6] = gt_boxes3d_ct[:, 6] - roi_ry
        pts_input = kitti_utils.rotate_pc_along_y_torch(Tensor.from_numpy(pts_input), Tensor.from_numpy(roi_ry)).asnumpy()

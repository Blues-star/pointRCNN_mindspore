...
def rotate_pc_along_y_torch(pc, rot_angle):
    ...
    cosa = ops.cos(rot_angle).view(-1, 1)  # (N, 1)
    # sina = torch.sin(rot_angle).view(-1, 1)  # (N, 1)
    sina = ops.sin(rot_angle).view(-1, 1)  # (N, 1)

    raw_1:Tensor = ops.concat([cosa, -sina], axis=1)  # (N, 2)
    raw_2:Tensor = ops.concat([sina, cosa], axis=1)  # (N, 2)
    R:Tensor = ops.concat((raw_1.expand_dims(axis=1), raw_2.expand_dims(axis=1)), axis=1)  # (N, 2, 2)

    pc_temp = pc[:, :, [0, 2]]  # (N, 512, 2)

    pc[:, :, [0, 2]] = ops.matmul(pc_temp, R.transpose(0, 2, 1))  # (N, 512, 2)
...
def boxes3d_to_corners3d_torch(boxes3d, flip=False):
    ...
    zeros = ms.numpy.zeros((boxes_num, 1))
    # ones = torch.cuda.FloatTensor(boxes_num, 1).fill_(1)
    ones = ms.numpy.ones((boxes_num, 1))
    x_corners:Tensor = ops.concat([l / 2., l / 2., -l / 2., -l / 2., l / 2., l / 2., -l / 2., -l / 2.], axis=1)  # (N, 8)
    y_corners:Tensor = ops.concat([zeros, zeros, zeros, zeros, -h, -h, -h, -h], axis=1)  # (N, 8)
    z_corners:Tensor = ops.concat([w / 2., -w / 2., -w / 2., w / 2., w / 2., -w / 2., -w / 2., w / 2.], axis=1)  # (N, 8)
    corners = ops.concat((x_corners.expand_dims(axis=1), y_corners.expand_dims(axis=1), z_corners.expand_dims(axis=1)), axis=1) # (N, 3, 8)

    cosa, sina = ops.cos(ry), ops.sin(ry)
    raw_1 = ops.concat([cosa, zeros, sina], axis=1)
    raw_2 = ops.concat([zeros, ones, zeros], axis=1)
    raw_3 = ops.concat([-sina, zeros, cosa], axis=1)
    R = ops.concat((raw_1.expand_dims(axis=1), raw_2.expand_dims(axis=1), raw_3.expand_dims(axis=1)), axis=1)  # (N, 3, 3)
    
    corners_rotated = ops.matmul(R, corners)  # (N, 3, 8)
    # corners_rotated = corners_rotated + centers.expand_dims(axis=2).expand(-1, -1, 8)
    corners_rotated = corners_rotated + centers.expand_dims(axis=2)
    corners_rotated:Tensor = ops.broadcast_to(corners_rotated,(-1, -1, 8))
    corners_rotated = corners_rotated.transpose(0, 2, 1)

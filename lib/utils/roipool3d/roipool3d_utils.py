...
def roipool3d_gpu(pts, pts_feature, boxes3d, pool_extra_width, sampled_pt_num=512):
    ...
    forward = get_func_from_so(so_name,"roipool3d_gpu")
    forward(pts, pooled_boxes3d, pts_feature, pooled_features, pooled_empty_flag)
...
def pts_in_boxes3d_cpu(pts:ms.Tensor, boxes3d:ms.Tensor):
    ...
    pts_flag = ms.numpy.zeros((boxes3d.shape[0], pts.shape[0]), dtype=ms.numpy.int64)
    # print(111)
    # roipool3d_cuda.pts_in_boxes3d_cpu(pts_flag,pts, boxes3d)

    # pts_in_boxes3d_cpu = get_func_from_so(so_name,"pts_in_boxes3d_cpu")
    # pts_in_boxes3d_cpu(pts_flag, pts, boxes3d)
    # boxes_pts_mask_list = []
...
def roipool_pc_cpu(pts, pts_feature, boxes3d, sampled_pt_num):
    ...
    roipool3d_cpu = get_func_from_so(so_name,"roipool3d_cpu",-1,True)
...
def roipool3d_cpu(boxes3d, pts, pts_feature, pts_extra_input, pool_extra_width, sampled_pt_num=512,
    ...
    pooled_pts, pooled_features, pooled_empty_flag = roipool_pc_cpu(ms.Tensor.from_numpy(pts), ms.Tensor.from_numpy(pts_feature_all),ms.Tensor.from_numpy(pooled_boxes3d), sampled_pt_num)

    extra_input_len = pts_extra_input.shape[1]
    # ms.Tensor.asnumpy()
    sampled_pts_input = ms.ops.concat((pooled_pts, pooled_features[:, :, 0:extra_input_len]), axis=2).asnumpy()
